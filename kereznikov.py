"""
Kereznikov — CS 1.6 hack
ESP через стены + AIM через мышь
"""
        ...
        if S['aim'] and ok:
            try:
                ...
                if best:
                    ...
                    k   = CFG['strength']
                    move_mouse(max(-100,min(100, dy_*k)),
                               max(-100,min(100,-dp*k)))
            except: pass
        time.sleep(0.008)
...
def esp_loop():
    ...
    # Ищем окно CS по любому совпадению
    cs_hwnd = 0
    titles_found = []
    for _ in range(30):
        def _cb(hwnd, _):
            ...
            if any(x in t.lower() for x in ('counter','half-life','cstrike','valve','cs ')):
    ...
    while True:
        ...
        if S['esp'] and ok:
            try:
                ...
                ang = get_angles()
                for pos in get_bots():
                    # Рисуем бокс в любом случае (через стены)
                    rel = (pos[0]-mp[0], pos[1]-mp[1], pos[2]-mp[2]+36)
                    pr  = w2s(rel, ang, W, H)
                    if not pr: continue
                    sx,sy,dist = pr

                    bh = max(14, int(1400/max(dist,1)))
                    bw = max(8,  bh//2)
                    x1,y1 = sx-bw//2, sy-bh

                    # Прямоугольник (всегда — через стены)
                    surf = pygame.Surface((bw,bh), pygame.SRCALPHA)
                    surf.fill((255,60,60,25))
                    pygame.draw.rect(surf,(255,60,60,255),(0,0,bw,bh),2)
                    sc.blit(surf,(x1,y1))

                    # Линия от низа экрана к боту
                    pygame.draw.line(sc,(200,50,255,200),(W//2,H),(sx,sy),1)

                    # Дистанция
                    dt = fnt.render(f"{int(dist)}u", True,(255,255,255))
...
root = ctk.CTk()
root.title("Kereznikov"); root.geometry("340x600")
...
def do_attach():
    res,msg = attach()
    if res: dot.configure(text_color="#4f8"); slbl.configure(text="ATTACHED")
...
ctk.CTkButton(root,text="ATTACH TO CS 1.6",command=do_attach,
              ...
              font=("Arial",12,"bold"),height=38,corner_radius=8).pack(fill="x",padx=14,pady=(12,2))

dbg = ctk.CTkLabel(root,text="p=--  y=--  bots=--",font=("Courier",10),text_color="#444")
dbg.pack(pady=2)

# Подсказка про оконный режим
ctk.CTkLabel(root,
    text="ESP: запусти CS в оконном режиме  (-windowed в параметрах)",
    font=("Arial",9), text_color="#333").pack()
